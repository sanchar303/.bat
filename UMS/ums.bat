...
for /f "tokens=1-7 delims=:" %%a in (users\%username%.txt) do (
    echo %%a:%newPassword%:%%c:%%d:%%e:%%f:%%g > "users\%username%.txt"
)
...
for /f "tokens=1-7 delims=:" %%a in (users\%username%.txt) do (
    echo %%a:%%b:%newEmail%:%%d:%%e:%%f:%%g > "users\%username%.txt"
)
...
for /f "tokens=1-7 delims=:" %%a in (users\%username%.txt) do (
    echo %%a:%%b:%%c:%newPhone%:%%e:%%f:%%g > "users\%username%.txt"
)
...
set "username=%~1"
cls
move /Y users\%username%.txt bin\%username%.txt >nul
echo %username% deleted successfully!
timeout /t 2 >nul
call :menu

:edit
start /B notepad users\%~1.txt >nul
call :viewProfile %~1
